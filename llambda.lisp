...
(defun dot-product-with-f16-tensor-row (row-pointer vector element-count)
  ...
  (let ((sum 0.0f0))
    (declare (single-float sum))
    (dotimes (index element-count (coerce sum 'single-float))
      (declare (fixnum index)
               (optimize (speed 3) (safety 0)))
      (incf sum (* (aref vector index)
                  (read-fp16-le row-pointer (* index 2)))))))

(declaim (inline get-scale-min-k4-at dot-product-with-k-m-tensor-body))
...
(defun dot-product-with-k-m-tensor-body (pointer qs-base vector vector-base
  ...
  (let ((sum 0.0f0)
    ...
    (declare (single-float sum)
            (fixnum qs-index low-index high-index))
    (dotimes (index 32 (the single-float sum))
           ...
           (multiple-value-bind (scale0 min0)
             ...
             (multiple-value-bind (scale1 min1)
               ...
               (let ((delta0 (* d scale0))
                 ...
                 (incf sum
                       (dot-product-with-k-m-tensor-body row-pointer
                                                         q-base
                                                         vector
                                                         (+ vector-base chunk-base)
                                                         delta0
                                                         offset0
                                                         delta1
                                                         offset1)))
...
(defun dot-product-with-q6-k-tensor-row (row-pointer vector element-count)
  ...
  (let ((sum 0.0f0)
    ...
    (dotimes (block-index block-count (coerce sum 'single-float))
      (declare (fixnum block-index))
      (let* ((block-offset (* block-index block-size))
             (d (read-fp16-le row-pointer (+ block-offset (- block-size 2))))
             (ql-pointer (cffi:inc-pointer row-pointer block-offset))
             (qh-pointer (cffi:inc-pointer row-pointer (+ block-offset (/ +qk-k+ 2))))
             (scales-pointer (cffi:inc-pointer row-pointer
                                               (+ block-offset (/ +qk-k+ 2) (/ +qk-k+ 4))))
             (vector-base (* block-index +qk-k+)))
        ...
        (dotimes (chunk-index (/ +qk-k+ 128))
          ...
          (let ((chunk-base (* chunk-index 128))
            ...
            (flet ((accumulate-lane (lane delta1 delta2 delta3 delta4)
                     (declare (fixnum lane)
                              (single-float delta1 delta2 delta3 delta4))
                     (let* ((packed-low (cffi:mem-aref ql-pointer :unsigned-char (+ ql-base lane)))
                            ...
                            (q4 (- (logior (ash packed-high -4)
                                   ...
                                   32))
                            (index1 (+ vector-base chunk-base lane))
                            (index2 (+ index1 32))
                            (index3 (+ index1 64))
                            (index4 (+ index1 96)))
                       (declare (fixnum q1 q2 q3 q4 index1 index2 index3 index4))
              ...
              (let ((delta1 (* d (read-s8 scales-pointer (+ scale-base 0))))
                    (delta2 (* d (read-s8 scales-pointer (+ scale-base 2))))
                    (delta3 (* d (read-s8 scales-pointer (+ scale-base 4))))
                    (delta4 (* d (read-s8 scales-pointer (+ scale-base 6)))))
                (declare (single-float delta1 delta2 delta3 delta4))
                (dotimes (lane 16)
                  (declare (fixnum lane))
                  (accumulate-lane lane delta1 delta2 delta3 delta4)))
              (let ((delta1 (* d (read-s8 scales-pointer (+ scale-base 1))))
                    (delta2 (* d (read-s8 scales-pointer (+ scale-base 3))))
                    (delta3 (* d (read-s8 scales-pointer (+ scale-base 5))))
                    (delta4 (* d (read-s8 scales-pointer (+ scale-base 7)))))
                (declare (single-float delta1 delta2 delta3 delta4))
                (dotimes (lane-offset 16)
                  (declare (fixnum lane-offset))
                  (accumulate-lane (+ 16 lane-offset)
                                   delta1
                                   delta2
                                   delta3
                                   delta4))))))))))
...
(defun dot-product-with-tensor-row (mapping tensor-info row-index vector)
  ...
  (let* ((column-count (tensor-column-count tensor-info))
    ...
    (case type-tag
      ...
      (let ((sum 0.0f0))
        (declare (single-float sum))
         (dotimes (index column-count (coerce sum 'single-float))
           (declare (fixnum index))
           (incf sum (* (aref vector index)
                        (cffi:mem-aref row-pointer :float index))))))
      (#.+ggml-type-f16+
       (dot-product-with-f16-tensor-row row-pointer vector column-count))
      (#.+ggml-type-bf16+
       (let ((sum 0.0f0))
         (declare (single-float sum))
         (dotimes (index column-count (coerce sum 'single-float))
           (declare (fixnum index))
           (incf sum (* (aref vector index)
                        (read-bf16-le row-pointer (* index 2)))))))
      (#.+ggml-type-q4-k+
      (dot-product-with-q4-k-m-tensor-row row-pointer vector column-count))
      (#.+ggml-type-q6-k+
      (dot-product-with-q6-k-tensor-row row-pointer vector column-count))
      (otherwise
       (dot-product vector
  ...
  (let* ((tensor-info (gguf-model-tensor-info model tensor-name t))
    ...
    (dotimes (row-index row-count dest)
      ...
      (setf (aref dest row-index)
            (dot-product-with-tensor-row (gguf-model-mapping model)
                                         tensor-info
                                         row-index
                                         vector)))))
